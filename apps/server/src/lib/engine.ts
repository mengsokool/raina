...
async function executeActionNodeLegacy(
  node: blocks.GraphNode,
  ctx: AutomationContext,
  projectId: string,
  automationId: string,
  graph: blocks.AutomationGraph,
  runId?: string
): Promise<{ status: "ok" | "error"; detail?: string; output?: unknown; paused?: boolean }> {
  const config = node.config || {};

  if (node.kind === "set_variable") {
    const varKey = String(config.variable || "");
    if (!varKey) {
      return { status: "error", detail: "Missing target variable" };
    }

    let rawVal = config.value;
    if (typeof rawVal === "string") {
      rawVal = interpolate(rawVal, {
    ...
    const parsedVal = !isNaN(numVal) && isFinite(numVal) ? numVal : rawVal;
    const strVal = String(parsedVal);
    const now = BigInt(Date.now());

    const deviceId = ctx.deviceId || (await getOrCreateDefaultDevice(projectId));
    ...
    await prisma.projectVariable.upsert({
      ...
      create: {
        id: `var_${projectId}_${deviceId}_${varKey}`,
        projectId,
        deviceId,
        key: varKey,
        value: strVal,
    ...
    if (currentDepth < 5) {
      void evaluateVariableAutomations(projectId, varKey, parsedVal, deviceId, currentDepth + 1);
    }

    return {
      status: "ok",
      detail: `Set ${varKey} = ${strVal}`,
      output: { variable: varKey, value: parsedVal, deviceId },
    };
  }

  if (node.kind === "call_integration") {
    const integrationId = String(config.integration_id || config.integration || "");
    if (!integrationId) {
      return { status: "error", detail: "Missing integration_id" };
    }

    const integrationRow = await prisma.integration.findFirst({
      where: { id: integrationId, projectId, archivedAt: null },
    });

    if (!integrationRow) {
      return { status: "error", detail: `Integration not found: ${integrationId}` };
    }

    if (!integrationRow.enabled) {
      return { status: "ok", detail: "Integration skipped (disabled)" };
    }

    const decryptedConfig = await openIntegrationConfig(integrationRow.config);

    const integrationContext: integrations.IntegrationContext = {
      source: ctx.source,
      projectId,
      ts: ctx.ts,
      variable: ctx.variable,
    ...
    const invocation: integrations.IntegrationInvocation = {
      ...
      params: (config.params as Record<string, unknown> | undefined) || (config as Record<string, unknown>),
    };

    const result = await integrations.executeIntegration(
      {
        id: integrationRow.id,
        project_id: integrationRow.projectId,
        name: integrationRow.name,
        kind: integrationRow.kind,
        config: decryptedConfig,
        enabled: integrationRow.enabled ? 1 : 0,
      },
      integrationContext,
      invocation
    );
    ...
    return {
      ...
      detail: result.detail || `${integrationRow.name} (${integrationRow.kind}) executed`,
      output: result,
    };
  }

  if (node.kind === "emit_event") {
    const rawEvent = String(config.event || "automation_event");
    const eventName = interpolate(rawEvent, {
      variable: ctx.variable,
      value: ctx.value,
      source: ctx.source,
      ...(ctx.payload || {}),
    ...
    const currentDepth = ctx.depth || 0;
    if (currentDepth < 3) {
      void triggerEventAutomations(projectId, eventName, {
        ...ctx,
        source: "event",
        event: eventName,
    ...
    return {
      status: "ok",
      detail: `Emitted event "${eventName}"`,
      output: { event: eventName },
    };
  }

  if (node.kind === "delay") {
    const amount = Math.max(1, Number(config.delay_amount || 1));
    const unit = String(config.delay_unit || "seconds");

    let delayMs = amount * 1000;
    if (unit === "minutes") delayMs = amount * 60 * 1000;
    else if (unit === "hours") delayMs = amount * 3600 * 1000;

    if (delayMs <= 5000 || ctx.isManual) {
      const waitTime = Math.min(delayMs, 5000);
      await new Promise((r) => setTimeout(r, waitTime));
      return {
        status: "ok",
        detail: `Waited ${amount} ${unit} (inline)`,
        paused: false,
      };
    }
    ...
    for (const edge of outgoingEdges) {
      const fireAt = BigInt(Date.now() + delayMs);
    ...
    return {
      status: "ok",
      detail: `Scheduled resumption in ${amount} ${unit}`,
      paused: true,
      output: { fireAt: Date.now() + delayMs },
    };
  }

  return { status: "ok", detail: `Block ${node.kind} completed` };
}
